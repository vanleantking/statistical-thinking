...
Aside from hypothesis testing, one of the most common uses of statistical inference is the estimation of unknown parameters using sample data. Polling is one example where statistical estimation is used. For example, [Gallup](http://www.gallup.com/) and the [Pew Research Center](http://www.pewresearch.org/) are organizations that carry out public opinion research and then use statistical estimation to provide snapshots of public attitudes and opinions on topics from politics and the economy, to social awareness and health and well-being. The results of their polls are seen on a daily basis in almost every newspaper, news blog and website the world over.

Statistical estimation is used by more than pollsters. Biologists, social scientists, and medical researchers use statistical estimation to quantify population characteristics. For example, the Minnesota Department of Natural Resources estimates the size of animal, bird, and fish populations yearly. These estimates are used to help set hunting and fishing regulations, as well as to allocate resources.^[Here is the [Wolf Population report for 2016](http://files.dnr.state.mn.us/wildlife/wolves/2016/survey-wolf.pdf).]
...
In the popular media, interval estimates are often reported using the sample estimate (statistic) and a margin of error. For example, consider the following poll results reported in the [New York Times on June 30, 2011](http://query.nytimes.com/gst/fullpage.html?res=980DE0DF1530F933A05755C0A9679D8B63):
...
- It is available at [http://www.amstat.org/sections/srms/pamphlet.pdf](http://www.amstat.org/sections/srms/pamphlet.pdf).
